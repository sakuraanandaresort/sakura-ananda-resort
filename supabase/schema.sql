...
alter table public.reservations add column if not exists notification_consent boolean not null default true;
alter table public.reservations add column if not exists room_name text;

-- Keep a human-readable room name on each reservation so external integrations
-- (such as Google Sheets) do not need a privileged Supabase query.
create or replace function public.set_reservation_room_name()
returns trigger
language plpgsql
security definer
...
drop trigger if exists reservations_room_name on public.reservations;
create trigger reservations_room_name
before insert or update of room_id on public.reservations
for each row execute function public.set_reservation_room_name();

update public.reservations r
set room_name = rooms.name
from public.rooms
where rooms.id = r.room_id and (r.room_name is null or r.room_name = '');
...
-- Public reservation creation is handled by a narrow SECURITY DEFINER RPC.
-- This avoids exposing reservation SELECT access to anonymous guests while still
-- allowing the public booking form to perform an atomic availability check + insert.
...
begin
  ...
  if coalesce(trim(p_mobile),'') = '' then raise exception 'Mobile number is required.'; end if;
  if p_check_out <= p_check_in then raise exception 'Check-out must be after check-in.'; end if;
  if p_guests is null or p_guests < 1 then raise exception 'Number of guests must be at least 1.'; end if;
  ...
  if p_guests > r.max_guests then
    raise exception 'Maximum guests for this room is %.', r.max_guests;
  end if;

  -- Serialize bookings for this room so two guests cannot reserve the same dates simultaneously.
  ...
  if existing_id is not null then
    raise exception 'That room is already reserved for part of those dates.';
  end if;

  v_nights := (p_check_out - p_check_in);
  v_total := v_nights * r.rate;
  v_deposit := greatest(coalesce(p_deposit,0),0);
  if v_deposit > v_total then v_deposit := v_total; end if;

  insert into public.reservations(
    guest_name,mobile,email,check_in,check_out,guests,room_id,
    rate_per_night,nights,room_total,deposit,balance,payment_method,
    payment_ref,payment_proof_url,status,special_requests,notification_consent
  ) values (
    trim(p_guest_name),trim(p_mobile),nullif(trim(coalesce(p_email,'')),''),
    p_check_in,p_check_out,p_guests,p_room_id,r.rate,v_nights,v_total,
    v_deposit,greatest(v_total-v_deposit,0),coalesce(nullif(trim(p_payment_method),''),'GCash'),
    nullif(trim(coalesce(p_payment_ref,'')),''),nullif(trim(coalesce(p_payment_proof_url,'')),''),
    'Pending',nullif(trim(coalesce(p_special_requests,'')),''),coalesce(p_notification_consent,true)
  ) returning reservations.booking_id into v_booking_id;

  return query select v_booking_id;
end;
$$;

revoke all on function public.create_public_reservation(text,text,text,date,date,integer,uuid,numeric,text,text,text,text,boolean) from public;
grant execute on function public.create_public_reservation(text,text,text,date,date,integer,uuid,numeric,text,text,text,text,boolean) to anon,authenticated;
...
-- Google Sheets / Gmail bridge without using Supabase Dashboard Webhooks.
-- This uses Supabase's pg_net extension to call the Google Apps Script Web App
-- asynchronously whenever a reservation is inserted or updated.
create extension if not exists pg_net with schema extensions;
create extension if not exists vault with schema vault;

create or replace function public.sakura_google_sheets_sync()
returns trigger
language plpgsql
security definer
...
begin
  ...
  where name = 'sakura_google_sheets_webhook_url'
  limit 1;

  if v_url is null or length(trim(v_url)) = 0 then
    raise warning 'Sakura Google Sheets webhook URL is not configured in Supabase Vault.';
    return new;
  end if;
...
exception when others then
  raise warning 'Sakura Google Sheets sync failed: %', SQLERRM;
  return new;
end;
$$;

drop trigger if exists reservations_google_sheets_sync on public.reservations;
create trigger reservations_google_sheets_sync
after insert or update on public.reservations
for each row execute function public.sakura_google_sheets_sync();
